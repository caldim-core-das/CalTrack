...
export function ApprovalCenterPage() {
  ...
  const activeEmployee = useMemo(() => {
    const dynamicStatus = dossier?.adminClearance?.status === "approved"
      ...
      : dossier?.adminClearance?.status === "rejected"
        ? "Rejected"
        : dossier?.adminClearance?.status === "activated"
          ? "Activated"
          : "Pending Review"
  ...
  async function confirmApprove() {
    try {
      ...
      alert(`${activeEmployee.name} Approved Successfully!\n\nInvitation email dispatched with token link.\nTechnician credentials created in inactive state.`)
    } catch (e) {
  ...
  async function handleRejectSubmit(e) {
    ...
    try {
      ...
      setRejectError("")
      alert(`Employee ${activeEmployee.name} Application Rejected. Rejection email sent.`)
    } catch (e) {
  ...
  // Queue: show the real registered employee from the dossier
        ...
        <div className="flex-1 overflow-y-auto p-8 pb-32 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl border ${activeEmployee?.regForm?.otpStatus === "verified"
              ...
              {/* Face Scan */}
              <div className={`flex items-center gap-3 p-3 rounded-xl border ${activeEmployee?.regForm?.isBiometricCompleted
                  ? "bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800/60"
                  : "bg-slate-50 dark:bg-slate-950/30 border-slate-200 dark:border-slate-800"
                }`}>
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl border ${activeEmployee?.regForm?.isCompleted
                  ? "bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800/60"
                  : "bg-slate-50 dark:bg-slate-950/30 border-slate-200 dark:border-slate-800"
                }`}>
          ...
          {/* Email & Invitation Telemetry */}
          {dossier?.adminClearance && dossier.adminClearance.status !== "pending" && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm font-sans">
              <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5 pb-3 border-b border-slate-100 dark:border-slate-800/80">
                <span className="text-indigo-500">8️⃣</span> Email & Invitation Telemetry
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold">
                <div className="p-3.5 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider">Recipient Address</span>
                  <span className="text-slate-800 dark:text-slate-200 truncate block">{activeEmployee.email}</span>
                </div>

                <div className="p-3.5 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider">Email Delivery Status</span>
                  <span className="inline-flex items-center gap-1.5 font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/10 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    {dossier?.adminClearance?.status === "rejected" ? "Rejection Dispatched" : dossier?.adminClearance?.invitationEmailStatus || "Delivered"}
                  </span>
                </div>
                ...
                {dossier?.adminClearance?.invitationToken && (
                  <>
                    <div className="p-3.5 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                      <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider">Invitation Dispatched</span>
                      <span className="text-slate-800 dark:text-slate-200 font-mono text-[10px] block">
                        {dossier.adminClearance.invitationSentAt ? new Date(dossier.adminClearance.invitationSentAt).toLocaleString("en-GB") : "Just now"}
                      </span>
                    </div>

                    <div className="p-3.5 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                      <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider">Expiration (24hr SLA)</span>
                      <span className="text-slate-800 dark:text-slate-200 font-mono text-[10px] block">
                        {dossier.adminClearance.invitationExpiresAt ? new Date(dossier.adminClearance.invitationExpiresAt).toLocaleString("en-GB") : "Tomorrow"}
                      </span>
                    </div>

                    <div className="col-span-2 p-3.5 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-indigo-500/20 relative group overflow-hidden">
                      <div className="flex justify-between items-center">
                        <div className="space-y-1 w-full min-w-0 pr-12">
                          <span className="block text-[8px] text-indigo-500 font-black uppercase tracking-wider">Secure Activation URL</span>
                          <span className="text-slate-800 dark:text-slate-200 font-mono text-[9px] block truncate select-all">
                        ...
                        <button
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors text-[10px] font-bold shadow-sm"
                        >
                          Copy Link
                        </button>
                      </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-sm">
            ...
            <div className="space-y-4 font-mono text-[10px] text-slate-500 dark:text-slate-400 leading-normal pl-4 border-l border-indigo-500/20">
              {dossier?.adminClearance?.auditLogs && dossier.adminClearance.auditLogs.length > 0 ? (
                dossier.adminClearance.auditLogs.map((log, idx) => (
                  <div key={idx} className="relative text-indigo-600 dark:text-indigo-400 font-bold">
                    <div className="absolute left-[-21px] top-1 w-2.5 h-2.5 rounded-full bg-indigo-600" />
                    <span className="text-slate-900 dark:text-white font-bold mr-3">{log.timestamp.split(" ").slice(3).join(" ")}</span>
                    <span className="text-slate-400 font-normal mr-2">[{log.action}]</span>
                    <span className="text-slate-700 dark:text-slate-300 font-semibold">{log.details}</span>
                  </div>
                ))
              ) : (
                <>
                  ...
                  {activeEmployee.status === "Invitation Sent" && (
                    <div className="relative text-emerald-600 dark:text-emerald-500 font-bold">
                      ...
                      <span className="font-bold mr-3">11:45</span> Approved By Admin (Invitation Dispatched)
                    </div>
                  )}
                  ...
                  {activeEmployee.status === "Activated" && (
                    <div className="relative text-emerald-600 dark:text-emerald-500 font-bold">
                      <div className="absolute left-[-21px] top-1 w-2.5 h-2.5 rounded-full bg-emerald-650" />
                      <span className="font-bold mr-3">12:00</span> Account Activated by Employee
                    </div>
                  )}
                </>
              )}
        ...
        <div className="w-[320px] border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0 overflow-y-auto p-5 space-y-4">
          ...
          {queueEmployees.length === 0 ? (
            ...
            <div
              ...
              className={`p-4 rounded-2xl border transition-all cursor-pointer ${selectedEmployeeId === emp.id
